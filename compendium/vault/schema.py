"""Default CLAUDE.md template generation.

The vault's CLAUDE.md is the schema contract every agent reads before
operating on the vault. It describes directory layout, page conventions,
operations, and writing style.

The template is a starter — users are expected to edit it over time as
conventions evolve for their specific topic. The topic and initial date
are interpolated at generation time; everything else is constant.
"""
...
CLAUDE_MD_TEMPLATE = """# Vault Schema — {topic}

This vault holds a compounding knowledge base on **{topic}**, maintained
...
<!--
  Free-form area. Co-evolve this section with the LLM as conventions
  firm up for your specific topic:
  - What counts as an entity vs. a concept in this domain?
  - Which authorities are considered canonical?
  - Which controversies are active vs. settled?
  - Any naming conventions specific to this topic?
-->
"""


def render_claude_md(topic: str, *, date_str: str | None = None) -> str:
    """Render the default CLAUDE.md for a freshly-initialized vault."""
